...
trait HasWorkingCopy
{
    ...
    public function saveToWorkingCopy(array $attributes, ?User $user = null): Revision
    {
        ...
        $data = [
            'entry_id' => $this->id,
            'user_id' => $user?->id ?? auth()->id() ?? $this->author_id,
            'title' => array_key_exists('title', $attributes) ? $attributes['title'] : $this->title,
            'data' => array_key_exists('data', $attributes) ? $attributes['data'] : $this->data,
            'content' => array_key_exists('content', $attributes) ? $attributes['content'] : $this->content,
            'status' => array_key_exists('status', $attributes) ? $attributes['status'] : $this->status->value,
            'action' => 'working',
            'is_current' => false,
            'created_at' => now(),
        ];
