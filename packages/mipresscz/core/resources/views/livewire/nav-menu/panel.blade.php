{{-- ============================================================
     Nav Menu Panel Livewire Component (Sidebar)
============================================================ --}}
...
<div>
    ...
    <div class="fmm-panel-card">
        ...
        <div class="fmm-panel-tabs">
            ...
            >{{ __('content.menus.panel_tab_custom') }}</button>
        ...
        <div class="fmm-panel-body">
            ...
            @if(!$menuId)
                <p style="font-size:0.8rem;color:var(--fmm-muted);text-align:center;padding:1rem 0">
                    {{ __('content.menus.select_menu_first') }}
                </p>
            ...
            @elseif($activeTab === 'custom')
                ...
                <div>
                    ...
                    <input
                        ...
                        placeholder="{{ __('content.menus.custom_title_placeholder') }}"
                ...
                <div>
                    ...
                    <input
                        ...
                        placeholder="https://... {{ __('content.menus.or') }} /"
                        style="width:100%;padding:0.4rem 0.625rem;border-radius:0.375rem;border:1.5px solid var(--fmm-border);background:var(--fmm-input-bg);color:var(--fmm-text);font-size:0.8125rem;outline:none"
                    />
                </div>
                <div>
                    <div class="fmm-label">{{ __('content.menus.field_open_in') }}</div>
                    <select
            ...
            @elseif($activeTab === 'models')
                ...
                @if(empty($modelSources))
                    <div class="fmm-empty" style="padding:1.5rem 0">
                        <p style="font-size:0.8rem">{{ __('content.menus.no_model_sources') }}</p>
                    </div>
                @else
                    <div>
                        <div class="fmm-label">{{ __('content.menus.search') }}</div>
                        <input
                            type="text"
                            wire:model.live.debounce.300="modelSearch"
                            placeholder="{{ __('content.menus.search_placeholder') }}"
                            style="width:100%;padding:0.4rem 0.625rem;border-radius:0.375rem;border:1.5px solid var(--fmm-border);background:var(--fmm-input-bg);color:var(--fmm-text);font-size:0.8125rem;outline:none"
                        />
                    </div>
                    ...
                    @foreach($modelSources as $modelClass)
                        <div>
                            ...
                            <div class="fmm-model-list" style="margin-top:0.375rem">
                                ...
                                @empty
                                    <p style="font-size:0.75rem;color:var(--fmm-muted);padding:0.5rem 0">{{ __('content.menus.no_records') }}</p>
                                @endforelse
                            </div>
                        </div>
                    @endforeach
