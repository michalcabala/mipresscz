{{-- ============================================================
     Recursive Nav Menu Item Component
     Usage: @include('mipresscz-core::livewire.nav-menu.menu-item', compact('item','depth','index'))
============================================================ --}}
...
<div class="fmm-item-row" data-id="{{ $item['id'] }}">
    ...
    <div class="fmm-item-card {{ !$item['enabled'] ? 'disabled-item' : '' }}">
        ...
        {{-- Drag Handle --}}
        <span class="fmm-drag-handle" title="{{ __('content.menus.drag_to_reorder') }}">
        ...
        <div class="fmm-item-actions">
            ...
            {{-- Move Up --}}
            <button class="fmm-action-btn" wire:click="moveUp({{ $item['id'] }})" title="{{ __('content.menus.move_up') }}">
            ...
            {{-- Move Down --}}
            <button class="fmm-action-btn" wire:click="moveDown({{ $item['id'] }})" title="{{ __('content.menus.move_down') }}">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" style="width:0.875rem;height:0.875rem">
            ...
            {{-- Indent (→) --}}
            @php
                $maxDepthConfig = config('nav-menu.max_depth');
                $canIndent = true;
                if ($maxDepthConfig !== null) {
                    $getSubtreeDepth = function($items) use (&$getSubtreeDepth) {
                        $max = 0;
                        foreach ($items as $child) {
                            $d = 1 + $getSubtreeDepth($child['children'] ?? []);
                            if ($d > $max) { $max = $d; }
                        }
                        return $max;
                    };
                    $subtreeDepth = $getSubtreeDepth($item['children'] ?? []);
                    if (($depth + 1 + $subtreeDepth) > $maxDepthConfig) {
                        $canIndent = false;
                    }
                }
            @endphp
            @if($canIndent && $index > 0)
                <button class="fmm-action-btn" wire:click="indentItem({{ $item['id'] }})" title="{{ __('content.menus.indent') }}">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" style="width:0.875rem;height:0.875rem">
            ...
            {{-- Outdent (←) --}}
            @if($depth > 0)
                <button class="fmm-action-btn" wire:click="outdentItem({{ $item['id'] }})" title="{{ __('content.menus.outdent') }}">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" style="width:0.875rem;height:0.875rem">
            ...
            {{-- Toggle visibility --}}
            <button class="fmm-action-btn" wire:click="toggleEnabled({{ $item['id'] }})" title="{{ $item['enabled'] ? __('content.menus.disable') : __('content.menus.enable') }}">
                @if($item['enabled'])
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" style="width:0.875rem;height:0.875rem">
            ...
            {{-- Edit --}}
            <button class="fmm-action-btn" wire:click="startEdit({{ $item['id'] }})" title="{{ __('content.menus.edit') }}">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" style="width:0.875rem;height:0.875rem">
            ...
            {{-- Delete --}}
            <button class="fmm-action-btn danger" wire:click="confirmRemoval({{ $item['id'] }})" title="{{ __('content.menus.remove') }}">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" style="width:0.875rem;height:0.875rem">
    ...
    @if(isset($editingItemId) && $editingItemId == $item['id'])
        <div class="fmm-edit-form" style="margin-top:0.375rem">
            <div>
                <div class="fmm-label">{{ __('content.menus.field_title') }}</div>
                <input type="text" wire:model="editingTitle" placeholder="{{ __('content.menus.field_title_placeholder') }}" />
            </div>
            <div class="fmm-edit-row">
                <div style="flex:2">
                    <div class="fmm-label">{{ __('content.menus.field_url') }}</div>
                    <input type="text" wire:model="editingUrl" placeholder="https://..." />
                </div>
                <div style="flex:1">
                    <div class="fmm-label">{{ __('content.menus.field_target') }}</div>
                    <select wire:model="editingTarget">
                        <option value="_self">{{ __('content.menus.target_self') }}</option>
                        <option value="_blank">{{ __('content.menus.target_blank') }}</option>
                    </select>
                </div>
            </div>
            <div style="display:flex;align-items:center;gap:0.5rem">
                <input type="checkbox" wire:model="editingEnabled" id="enabled-{{ $item['id'] }}" style="width:auto" />
                <label for="enabled-{{ $item['id'] }}" class="fmm-label" style="margin:0;cursor:pointer">{{ __('content.menus.field_visible') }}</label>
            </div>
            <div class="fmm-edit-actions">
                ...
                >{{ __('content.menus.cancel') }}</button>
                <button
                    wire:click="saveEdit"
    ...
    @if(!empty($item['children']))
        <div class="fmm-nested-list fmm-nested-sortable" x-data="menuSortable($wire, {{ config('nav-menu.max_depth') ?? 'null' }})">
            @foreach($item['children'] as $childIdx => $child)
                @include('mipresscz-core::livewire.nav-menu.menu-item', ['item' => $child, 'depth' => $depth + 1, 'index' => $childIdx])
            @endforeach
        </div>
    @else
        <div class="fmm-nested-list fmm-nested-sortable" x-data="menuSortable($wire, {{ config('nav-menu.max_depth') ?? 'null' }})"></div>
    @endif
