{{-- ============================================================
     Nav Menu Builder Livewire Component
============================================================ --}}
...
@if(!$hasMenu)
    <div class="fmm-builder-card">
        <div class="fmm-empty">
            ...
            <p class="text-sm font-semibold">{{ __('content.menus.no_menu_selected') }}</p>
            <p class="text-xs">{{ __('content.menus.no_menu_hint') }}</p>
        </div>
    </div>
@else
    <div class="fmm-builder-card">
        ...
        <div class="fmm-builder-header">
            <span>{{ __('content.menus.items_heading') }}</span>

            @if($autoSave)
                <span style="font-size:0.7rem;font-weight:500;color:var(--fmm-muted);">
                    ● {{ __('content.menus.auto_save_on') }}
                </span>
            @endif
        </div>
        ...
        >
            @forelse($items as $idx => $item)
                @include('mipresscz-core::livewire.nav-menu.menu-item', ['item' => $item, 'depth' => 0, 'index' => $idx])
            @empty
                <div class="fmm-empty" style="padding:2rem 1rem;">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
                    </svg>
                    <p class="text-xs">{{ __('content.menus.add_items_hint') }}</p>
                </div>
            @endforelse
        </div>
                    ...
                    <div style="padding: 1.5rem; display: flex; gap: 1rem; align-items: flex-start;">
                        ...
                        <div style="flex: 1;">
                            <h3 style="font-size: 1rem; font-weight: 600; color: var(--fmm-text); margin: 0;">
                                {{ __('content.menus.delete_item_title') }}
                            </h3>
                            <div style="margin-top: 0.5rem;">
                                <p style="font-size: 0.875rem; color: var(--fmm-muted);">
                                    {{ __('content.menus.delete_item_confirm') }}
                                </p>
                            </div>
                        </div>
                    </div>
